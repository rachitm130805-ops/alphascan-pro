...
# --- CURATED SCREENS ENDPOINT REGISTRY ---
CURATED_SCREENS = {
...
def fetch_live_screen_data(base_url, page=1):
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            if dfs:
                df = dfs[0]
                # Clean company names if combined with badges or newlines
...
# ==============================================================================
# TAB 1: CURATED SCREENS HUB (LIVE FULL DATABASE EXTRACTION)
# ==============================================================================
with tab_screens:
...
with tab_funda:
    ...
    with st.spinner(f"Aggregating verified reports for {stock_sym}..."):
        try:
            ...
            # 3. Dynamic Sector Peer Comparison (ACCURATE INDUSTRY PEERS)
...
# ==============================================================================
# TAB 5: INSTITUTIONAL CUSTOM SCREENER (REAL WHOLE-MARKET QUERY ENGINE)
# ==============================================================================
with tab_custom:
    st.markdown("<br>", unsafe_allow_html=True)
    ...
    if st.button("🚀 Run Live Market-Wide Query", key="btn_run_wide_query", use_container_width=True):
        # Formulate full SQL query string
        q_parts = [
